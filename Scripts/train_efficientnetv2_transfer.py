...
Features:
- Uses torchvision EfficientNetV2 (v2_s / v2_m / v2_l) pretrained on ImageNet; replaces classifier for 2 classes.
- Optional freezing of the feature extractor for initial epochs.
...
- Grad-CAM heatmaps for a sample of validation images.

Usage:
  # From single folder (random splits)
  python SCRIPTS/train_efficientnetv2_transfer.py \
      --data-root DATA/raw_data/cleaned_data \
      --epochs 10 --batch-size 64 --out-dir MODELS --arch v2_s

  # From existing split folders (train/val/test)
  python SCRIPTS/train_efficientnetv2_transfer.py \
  ...
  python SCRIPTS/train_efficientnetv2_transfer.py \
      --split-root dataset_split --gradcam --gradcam-samples 12 --arch v2_s

  # Dry-run to sanity-check a forward/backward step
  python SCRIPTS/train_efficientnetv2_transfer.py --dry-run --limit-per-class 8 --num-workers 0 --arch v2_s
"""
...
def replace_classifier_sequential(seq: nn.Sequential, num_classes: int):
    ...
    if last_linear_idx is None:
        raise RuntimeError("No nn.Linear found in classifier to replace.")


def create_efficientnet_v2(arch: str = "v2_s", num_classes: int = 2, pretrained: bool = True, freeze_features: bool = False):
    arch = arch.lower()
    model = None
    if arch == "v2_s":
        try:
        ...
        try:
            weights = torchvision.models.EfficientNet_V2_M_Weights.IMAGENET1K_V1 if pretrained else None
            model = torchvision.models.efficientnet_v2_m(weights=weights)
        except Exception:
            model = torchvision.models.efficientnet_v2_m(pretrained=pretrained)
    elif arch == "v2_l":
        try:
            weights = torchvision.models.EfficientNet_V2_L_Weights.IMAGENET1K_V1 if pretrained else None
            model = torchvision.models.efficientnet_v2_l(weights=weights)
        except Exception:
            model = torchvision.models.efficientnet_v2_l(pretrained=pretrained)
    else:
        raise ValueError("Unknown --arch. Choose from: v2_s, v2_m, v2_l")

    # Replace classifier head for 2 classes
    if isinstance(model.classifier, nn.Sequential):
        replace_classifier_sequential(model.classifier, num_classes)
    elif isinstance(model.classifier, nn.Linear):
        in_features = model.classifier.in_features
        model.classifier = nn.Linear(in_features, num_classes)
    else:
        raise RuntimeError("Unexpected classifier type; update the replacement logic.")
...
def generate_gradcam_samples(model: nn.Module, val_loader: DataLoader, device, out_dir: Path, num_samples: int = 8):
    ensure_dir(out_dir)
    # EfficientNetV2 exposes a .features module; use its last Conv2d
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune EfficientNetV2 with optional Grad-CAM visualization")
    parser.add_argument("--arch", type=str, default="v2_s", choices=["v2_s", "v2_m", "v2_l"], help="EfficientNetV2 variant")
    ...
    parser.add_argument("--gradcam", action="store_true", help="Generate Grad-CAM images on val set after training")
    ...
    gradcam_dir = report_dir/"gradcam_efficientnetv2"; ensure_dir(gradcam_dir)
    ...
    model = create_efficientnet_v2(arch=args.arch, num_classes=2, pretrained=True, freeze_features=args.freeze_features).to(device)
    ...
    if args.dry_run:
        ...
        print(f"EfficientNet{args.arch.upper()} dry-run ok. Loss=", float(loss.item()))
        return

    best_val_acc = 0.0
    best_model_path = out_dir / f"efficientnet_{args.arch}_best.pth"
    metrics_csv = out_dir / f"efficientnet_{args.arch}_metrics.csv"
    ...
    save_confusion_matrix(y_true, y_pred, class_names=list(CLASSES), out_path=report_dir/f"efficientnet_{args.arch}_confusion_matrix.png")
    report_txt = report_dir / f"efficientnet_{args.arch}_classification_report.txt"
